...
public abstract class CreateWorldScreenGameTabMixin {
    ...
    @WrapOperation(method = "<init>", at = @At(value = "INVOKE", target = "Lnet/minecraft/client/gui/screens/worldselection/WorldCreationUiState;addListener(Ljava/util/function/Consumer;)V", ordinal = 2))
    private void addKeepInvButton(WorldCreationUiState instance, Consumer<WorldCreationUiState> action, Operation<Void> original, @Local GridLayout.RowHelper helper) {
        original.call(instance, action);
        GameRules gameRules = instance.getGameRules();
        CycleButton<Boolean> cyclingButtonWidget = helper.addChild(CycleButton.onOffBuilder(gameRules.get(GameRules.KEEP_INVENTORY))
