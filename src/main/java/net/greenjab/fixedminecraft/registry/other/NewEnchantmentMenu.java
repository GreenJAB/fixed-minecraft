    ...
    private List<EnchantmentInstance> getEnchantmentList(final RegistryAccess access, final ItemStack itemStack, final int slot, final int enchantmentCost) {
        ...
        if (tag.isEmpty()) {
            ...
            enchantments.forEach((enchantment, level) -> {
                if (!itemStack.is(Items.BOOK) || enchantment.value().getMaxLevel()>1) enchantmentsResult.add(new EnchantmentInstance(enchantment, (FixedMinecraft.SERVER.getGameRules().get(GameRuleRegistry.SUPER_ENCHANT_CHANCE)>0&&itemStack.is(ItemTags.PIGLIN_LOVED)&&enchantment.value().getMaxLevel()!=1)?level+1:level));
            });
