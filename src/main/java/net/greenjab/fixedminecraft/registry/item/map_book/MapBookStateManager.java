...
public class MapBookStateManager {
    ...
    private final PersistentState.Type<MapBookState> persistentStateType = new PersistentState.Type<>(
    ...
    public MapBookState getMapBookState(MinecraftServer server, int id) {
        return server.getOverworld().getPersistentStateManager().get(this.persistentStateType, this.getMapBookName(id));
    }

    public void putMapBookState(MinecraftServer server, int id, MapBookState state) {
        server.getOverworld().getPersistentStateManager().set(getMapBookName(id), state);
    }
