...
object HUDOverlayHandler {
    ...
    fun drawHungerOverlay(
        ...
        for (i in startFoodBars until endFoodBars) {
            ...
            val iconSprite =
                TextureHelper.getFoodTexture(useRottenTextures, if (isHalf) TextureHelper.FoodType.HALF else TextureHelper.FoodType.FULL)
    ...
    fun onClientTick() {
        ...
        flashAlpha = Math.max(0F, Math.min(1F, unclampedFlashAlpha)) * Math.max(
            0F,
            Math.min(1F, 0.65f/*ModConfig.INSTANCE.maxHudOverlayFlashAlpha*/)
        )
    }
